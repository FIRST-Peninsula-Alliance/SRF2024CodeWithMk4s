...
public class SwerveSubsystem extends SubsystemBase {
    ...
    private boolean m_isFieldOriented = true;       // Field Oriented is the default
    // m_varMaxOutputFactor is a temporary driver settable joystick speed reduction,
    // which when triggered (by RightBumper) will be set to something 
    // between .25 and .5 (see ButtonBindings in RobotContainer for setting)
    // for translate, strafing, and rotating. Normal value is 1.0
    private double m_varMaxOutputFactor = 1.0;
    // FixedMaxTranslationOutput and FixedMaxRotationOutput are factors which will limit
    // the Max speeds requested from motor controllers, independent of m_varMaxOutputFactor,
    // and changable only via edit and re-compile. They are primarily to improve controllability
    // of the robot and reduce chance of damage, especially with inexperienced drivers, but 
    // also this can help to pin down whether excessive acceleration and current draw is 
    // causing problems.
    private double m_fixedMaxTranslationOutput  = SDC.OUTPUT_DRIVE_LIMIT_FACTOR;
    private double m_fixedMaxRotationOutput     = SDC.OUTPUT_ROTATE_LIMIT_FACTOR;

    // Network table entry handles to more efficiently publish SwerveDrive data:
    ...
    /***********************************************
     * Constructor for SwerveSubsystem
     **********************************************/
    ...
    // The following five methods establish the center of rotation, which can be set
    // on the fly, to either the center of the robot (default) or to one of the 
    // four corneers (i.e. centered on a specified module wheel).
    ...
    public void drive(Translation2d translation, double rotation, boolean isOpenLoop) {
        ...
        SwerveModuleState[] swerveModuleStates =
            SDC.SWERVE_KINEMATICS.toSwerveModuleStates(
                                        m_isFieldOriented 
                                        ? 
...
*/
    ...
    public void publishSwerveDriveData() {
        m_gyroYawEntry.setString(F.df2.format(getYaw2d().getDegrees()));
        m_gyroRawYawEntry.setString(F.  df2.format(m_gyro.getYaw().getValueAsDouble()));
        m_odometryHeadingEntry.setString(F.df2.format(m_gyro.getRotation2d().getDegrees()));
    ...
    // but could be useful for test purposes.
    ...
    // This method starts all drive motors spinning slowly with a positive
    // duty cycle value (set in SwerveModule to 0.2). They should all be moving CCW with 
    // the wheel rotation viewed from the left (beveled gear) side. 
    // The standard for swerve calibration of absolute encoder offsets requires the bevel 
    // gears be on the left when the robot is facing straight downfield (i.e. a heading of 0.0).
    // That same orientation should be set when using TunerX to determine positive direction of 
    // rotation of each swerve module wheel.
    public void testDriveMotorsRotation() {
        for(SwerveModule mod : m_swerveMods) {
            mod.testDriveMotorRotation();
        }
    }

    // This method starts all steering motors spinning slowly with a positive
    // duty cycle value. All modules should be rotating CCW viewed from the top.
    public void testSteerMotorsRotation() {
        for(SwerveModule mod : m_swerveMods) {
            mod.testSteerMotorRotation();
        }
    }
